...
{
    // ════════════════════════════════════════════════════════════════════
    //  重要：本文件所有枚举都未在 BatteryDbContext 里配置 HasConversion&lt;string&gt;，
    //  EF Core 默认按声明顺序的整数序号存库。这意味着任何枚举只要在中间插入新成员，
    //  后面所有成员的存储值都会整体错位，等价于污染所有历史数据（旧记录读出来的枚举
    //  值全部对不上）。新增枚举成员一律追加到对应枚举的最后一个成员之后，
    //  绝不要在中间插入或调整已有成员的顺序。
    // ════════════════════════════════════════════════════════════════════
    /// <summary>
    /// 工步类型 - 锂电池充放电基础工步
